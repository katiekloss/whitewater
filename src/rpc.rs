...
use whitewater::{RpcLifecycle, SetRequest};

pub struct RpcServer {
    pub rpc_channel: Sender<RpcLifecycle>
}
...
impl RpcServer {
    pub async fn listen_rpc(self) -> io::Result<()> {
        ...
        loop {
            ...
            let chan_inner = self.rpc_channel.clone();
            tokio::spawn(async move {
                Self::handle_connection(conn, addr, chan_inner).await;
            });
        }
    }
